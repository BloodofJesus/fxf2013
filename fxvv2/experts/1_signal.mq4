...
extern bool strategy_jpy = true;
extern bool strategy_nzdaud = true;
extern bool strategy_eurgbpchf = true;
extern bool strategy_audnzdcad = true;
extern bool new_orders = false;
extern int magic = 1230;
extern double lotsize_jpy = 0.10;
extern double lotsize_nzdaud = 0.25;
extern double lotsize_eurgbpchf = 0.10;
extern double lotsize_audnzdcad = 0.10;
...
string infobox, orderbox, createbox;
...
//----
   ...
   strategy_usd();
   stratagy_audnzdcad();
   Comment("Build 1, ", orderbox, infobox, createbox);
   if (filesave)
      FileAppend("signals/signal1_" + Year() + "_" + Month() + "_" + Day() + "_" + Hour()  + ".txt", orderbox+infobox+createbox);
//----
   return(0);
  }
//+------------------------------------------------------------------+
                  ...
                  if((OrderOpenPrice()-ask)>(point*InitialTrailingStop))
                    {
                     ...
                     if((OrderStopLoss()>(ask+point*TrailingStop)) || (OrderStopLoss()==0))
                       {
                        infobox = StringConcatenate(orderbox, "\n", OrderSymbol(), ", Modify Sell");
...
int create_jpy_orders(int i, int j, int k, int l, int m)
{
      ...
      if (checking[i][0] == totalpoints && checkingb[i][0]) {
         ...
         if (cnt < 2) {
            if (new_orders) createorder(aPair[i], 1, lotsize_jpy, magic, build + checking[i][0] + ", " + checkingb[i][0], 
               0, 0);
         }
         ...
         cnt = orders + orders2 + orders3 + orders4;
         if (cnt < 2) {
            if (new_orders) createorder(aPair[i], -1, lotsize_jpy, magic, build + checking[i][1] + ", " + checkingb[i][1], 
               0, 0);
         }
      }
}
...
int strategy_usd()
{
   ...
   if (strategy_eurgbpchf) {
      direction_point(aPair[2], PERIOD_H1, EURUSD);
      mathmurry(aPair[2], EURUSD);
      direction_point(aPair[0], PERIOD_H1, USDCHF);
      mathmurry(aPair[0], USDCHF);
      direction_point(aPair[1], PERIOD_H1, GBPUSD);
      mathmurry(aPair[1], GBPUSD);
      ...
      //start average costing
      render_avg_costing(aPair[i], i, lotsize_eurgbpchf);
      //average costing ends
      
      if (checking[i][0] == 8 && checkingb[i][0]) {
         //condition for buy, check if we have buy for gbp, if yes, then dont do anything, else buy aud
         orders = CalculateOrdersTypeSymbol(aPair[j], magic, OP_SELL);
         orders2 = CalculateOrdersTypeSymbol(aPair[k], magic, OP_BUY);
         if (orders == 0 && orders2 == 0) {
            //create order for EURUSD - lotsize_eurgbpchf
            if (new_orders) createorder(aPair[i], 1, lotsize_eurgbpchf, magic, build + checking[i][0] + ", " + checkingb[i][0], 
               0, 0);
         }
      } else if (checking[i][1] == 8 && checkingb[i][1]) {
         orders = CalculateOrdersTypeSymbol(aPair[j], magic, OP_BUY);
         orders2 = CalculateOrdersTypeSymbol(aPair[k], magic, OP_SELL);
         if (orders == 0 && orders2 == 0) {
            if (new_orders) createorder(aPair[i], -1, lotsize_eurgbpchf, magic, build + checking[i][1] + ", " + checkingb[i][1], 
               0, 0);
         }
      }
      i = 0;
   ...
   if (strategy_nzdaud) {
      ...
      if (checking[5][0] == 8 && checkingb[5][0]) {
         ...
         if (orders == 0) {
            ...
            if (new_orders) createorder(aPair[5], 1, lotsize_nzdaud, magic, build + checking[5][0] + ", " + checkingb[5][0], 
               0, 0);
         }
         ...
         if (orders == 0) {
            //create order for AUDUSD - lotsize_nzdaud
            if (new_orders) createorder(aPair[5], -1, lotsize_nzdaud, magic, build + checking[5][1] + ", " + checkingb[5][1], 
               0, 0);
         }
      }
      ...
      if (checking[22][0] == 8 && checkingb[22][0]) {
         ...
         if (orders == 0) {
            //create order for AUDUSD - lotsize_nzdaud
            if (new_orders) createorder(aPair[22], 1, lotsize_nzdaud, magic, build + checking[22][0] + ", " + checkingb[22][0], 
               0, 0);
         }
         ...
         if (orders == 0) {
            ...
            if (new_orders) createorder(aPair[22], -1, lotsize_nzdaud, magic, build + checking[22][1] + ", " + checkingb[22][1], 
               0, 0);
         }
      }
...
int create_audnzdcad_orders(int i, int j)
{
      ...
      if (condition_buy) {
         ...
         cnt = orders;
         if (cnt < 1) {
            createorder(aPair[i], 1, lotsize_audnzdcad, magic, build + ZZ_1 + "/" + ZZ_2, 
               0, 0);
         }
      } else if (condition_sell) {
         orders = CalculateOrdersTypeSymbol(aPair[j], magic, OP_SELL);
