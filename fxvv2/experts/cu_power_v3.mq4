...
int closingonprofit(string symbol)
{
   ...
   infobox = infobox + "\nTotal Profit: " + totalprofit + 
   ", required profit: " + profit + ", required profit2: " + profit2 + 
   ...
   /*
   if (stoploss > 0) {
      if(type == 1 && bid < stoploss) {
         infobox = infobox + "\nClosing all buy orders since bid is less than stop loss point";
         //closeallorders(symbol);
         stoploss = 0;
      } else if (type == -1 && ask > stoploss) {
         infobox = infobox + "\nClosing all sell orders since ask is more than stop loss point";
         //closeallorders(symbol);
         stoploss = 0;
      }
   }*/
...
int lookupcurrency(string symbol)
{
      ...
      bool condition_top, condition_bottom;
         ...
         if (cci3 > cci3b  && cci3 > -100) {
            infobox = infobox + ": M15: Buy";
            build_logic = build_logic + ":1";
         } else if (cci3 < cci3b && cci3 < 100) {
            infobox = infobox + ": M15: Sell";
            build_logic = build_logic + ":2";
         } else {
