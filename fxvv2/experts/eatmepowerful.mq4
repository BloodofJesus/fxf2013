...
//----
   ...
   for (x = 0; x < ARRSIZE; x++) {
      ...
      infobox = infobox + "\naLookupSingle: " +aLookupSingle + ", aStrengthSingle: " + aStrengthSingle
         ...
         + ", close_on_loss: " + close_on_loss;
      if (indication == 1) {
         if (createneworders) {
            createorder(aPair[x], 1, lotnew, magic, "Indication Ask: " + MarketInfo(symbol, MODE_ASK), 0, 0);
         }
      } else if (indication == -1) {
         if (createneworders) {
            createorder(aPair[x], -1, lotnew, magic, "Indication Bid: " + MarketInfo(symbol, MODE_BID), 0, 0);
         }
      }
      if (condition_buy) {
      ...
      if (condition_buy 
         ...
         if (
            ...
            if (createneworders) {
               createorder(aPair[x], 1, lotnew, magic, "Eatmepowerful Ask: " + MarketInfo(symbol, MODE_ASK), 0, 0);
            }
         } else if (
            condition_sell 
            && val4a < val5a
            ...
            if (createneworders) {
               createorder(aPair[x], -1, lotnew, magic, "Eatmepowerful Bid: " + MarketInfo(symbol, MODE_BID), 0, 0);
            }
         }
   }
   Comment (infobox);
