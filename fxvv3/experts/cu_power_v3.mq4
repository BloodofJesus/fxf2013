...
extern int market_day1 = 1;
extern int market_hour_d1_h1 = 9;
extern int market_hour_d1_m1 = 28;
extern string market_hour_d1_currency = "GBP";

extern int market_day2 = 1;
extern int market_hour_d2_h1 = 12;
extern int market_hour_d2_m1 = 15;
extern string market_hour_d2_currency = "USD";

extern int market_day3 = 1;
extern int market_hour_d3_h1 = 15;
extern int market_hour_d3_m1 = 0;
extern string market_hour_d3_currency = "USD";
...
//----
   ...
   //if (opentime != Time[0]) {
      ...
      infobox = infobox + "\nChecking Conditions: current_currency1: " + current_currency1 + ", current_currency2: " + current_currency2;
      ...
      if (markethours) {
         ...
         //day 1
         if (Day() == market_day1 
         && (current_currency1 == market_hour_d1_currency || current_currency2 == market_hour_d1_currency)
         && (
               (
                  hour == market_hour_d1_h1 
                  && Minute() >= market_hour_d1_m1
               )
               ||
               (
                  hour + 1 == market_hour_d1_h1 + 1
               )
               ||
               (
                  hour + 2 == market_hour_d1_h1 + 2
               )
            )
         ) {
            infobox = infobox + "\nDay 1 Market Hour Started.";
            if (check == 1 && create_new_orders) {
               createorder(symbol, PERIOD_M15, 1, TimeframeToString(PERIOD_M15) + " Level 0", magic, 0);
            } else if (check == -1 && create_new_orders) {
               createorder(symbol, PERIOD_M15, -1, TimeframeToString(PERIOD_M15) + " Level 0", magic, 0);
            }
         } else {
            infobox = infobox + ":Day 1 Condition Not Met:";
         }
         
         
         //day 2
         if (Day() == market_day2 
         && (current_currency1 == market_hour_d2_currency || current_currency2 == market_hour_d2_currency)
         && (
               (
                  hour == market_hour_d2_h1 
                  && Minute() >= market_hour_d2_m1
               )
               ||
               (
                  hour + 1 == market_hour_d2_h1 + 1
               )
               ||
               (
                  hour + 2 == market_hour_d2_h1 + 2
               )
            )
         ) {
            infobox = infobox + "\nDay 2 Market Hour Started.";
            if (check == 1 && create_new_orders) {
               createorder(symbol, PERIOD_M15, 1, TimeframeToString(PERIOD_M15) + " Level 0", magic, 0);
            } else if (check == -1 && create_new_orders) {
               createorder(symbol, PERIOD_M15, -1, TimeframeToString(PERIOD_M15) + " Level 0", magic, 0);
            ...
            infobox = infobox + ":Day 2 Condition Not Met:";
         }
         
         
         //day 3
         if (Day() == market_day3 
         && (current_currency1 == market_hour_d3_currency || current_currency2 == market_hour_d3_currency)
         && (
               (
                  hour == market_hour_d3_h1 
                  && Minute() >= market_hour_d3_m1
               )
               ||
               (
                  hour + 1 == market_hour_d3_h1 + 1
               )
               ||
               (
                  hour + 2 == market_hour_d3_h1 + 2
               )
            )
         ) {
            infobox = infobox + "\nDay 3 Market Hour Started.";
            if (check == 1 && create_new_orders) {
               createorder(symbol, PERIOD_M15, 1, TimeframeToString(PERIOD_M15) + " Level 0", magic, 0);
            } else if (check == -1 && create_new_orders) {
               createorder(symbol, PERIOD_M15, -1, TimeframeToString(PERIOD_M15) + " Level 0", magic, 0);
            ...
            infobox = infobox + ":Day 3 Condition Not Met:";
         }
      } else {
         //initial condition
         if (check == 1 && create_new_orders) {
...
int lookupcurrency(string symbol)
{
      ...
      bool condition_top, condition_bottom;
         ...
         double val, val2, val3, val4, val5, val6, val7, 
         cci, ccib, ccic, cci2, cci2b, cci2c, cci3, cci3b, cci3c;
         double h1,h2,h3,h4,h5,h6;
         //double ibandlower = iBands(symbol, PERIOD_M15, 20, 2, 0, PRICE_CLOSE, MODE_LOWER,0);
         //double ibandupper = iBands(symbol, PERIOD_M15, 20, 2, 0, PRICE_CLOSE, MODE_UPPER,0);
         ...
         //ccib = iCCI(symbol, PERIOD_M1,45,PRICE_TYPICAL,1);
         //ccic = iCCI(symbol, PERIOD_M1,45,PRICE_TYPICAL,2);
         //cci2 = iCCI(symbol, PERIOD_M5,45,PRICE_TYPICAL,0);
         //cci2b = iCCI(symbol, PERIOD_M5,45,PRICE_TYPICAL,1);
         //cci2c = iCCI(symbol, PERIOD_M5,45,PRICE_TYPICAL,2);
         cci3 = iCCI(symbol, PERIOD_M15,45,PRICE_TYPICAL,0);
         cci3b = iCCI(symbol, PERIOD_M15,45,PRICE_TYPICAL,1);
         //cci3c = iCCI(symbol, PERIOD_M15,45,PRICE_TYPICAL,2);
         ...
         condition_buy = (
                     ...
                     //&& MarketInfo(symbol, MODE_BID) < ibandupper && MarketInfo(symbol, MODE_BID) > ibandlower
                     );
         condition_sell = (
                        //direction == -1 
                        //&& 
         ...
         /*infobox = infobox + "\n";
         infobox = infobox + "Band";
         build_logic = build_logic + "::B";
         if (MarketInfo(symbol, MODE_BID) < ibandupper && MarketInfo(symbol, MODE_BID) > ibandlower) {
            infobox = infobox + ": M15: Inside Band";
            build_logic = build_logic + ":1";
         } else {
            infobox = infobox + ": M15: Outside Band";
            build_logic = build_logic + ":0";
         }*/
