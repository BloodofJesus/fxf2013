...
extern bool average_costing = true;
extern bool trailingfun = true;
extern bool close_on_loss = false;
...
//----
   ...
   for (x = 0; x < ARRSIZE; x++) {
      ...
      if (
            ...
            if (createneworders) {
               createorder(aPair[x], 1, lotnew, magic, "Idea HS: " + MarketInfo(symbol, MODE_ASK), 300, 0);
            }
         } else if (
            condition_sell 
            && val4a < val5a
            ...
            if (createneworders) {
               createorder(aPair[x], -1, lotnew, magic, "Idea HS: " + MarketInfo(symbol, MODE_BID), 300, 0);
            }
         }
   }
